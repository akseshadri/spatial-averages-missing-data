%% PFig6d.m — Standard error from robustness analysis (Figure 6d)
%
% Reproduces Figure 6, panel (d) of:
...
%
% Companion to PFig6abc.m. Estimates the standard error (SE) of spatially
% averaged rainfall for individual months (April-November), using the
% area-weighted 1 x 1 deg dataset as "true" values (robustness check).
%
% The SE formula and procedure are the same as in PFig5.m:
%   SE = { beta^T (D1 + sigma_e^2 I) beta }^{1/2}
% but here the bias matrix D1 is computed with respect to the 1 x 1 deg
% area-weighted average rather than the 0.25 x 0.25 deg product.
%
% Results are comparable to Figure 5: SE exhibits similar monthly variation
% and the ratio SE / mean ISMR is approximately 4.3% (vs. 4.1% in Fig. 5).
% For ISMR: SE ~ 0.33 mm/day, mean ~ 7.7 mm/day, stdev ~ 0.43 mm/day.
%
% Data:
...
%% Input periods
monthstartlist = [4 5 6 7 8 9 10 11 6 4];
monthendlist = [4 5 6 7 8 9 10 11 9 11];
...
for maincount = 1:numel(monthstartlist)
    ...
    for sigmaecount = 1:numel(sigmaelist)
        ...
        if maincount == 9
            sigmae = sigmae / sqrt(4);
        elseif maincount == 10
            sigmae = sigmae / sqrt(8);
        end
        ...
        for i = 1:size(precipp,1)
            dmi = precipp(i,:)'- ismr(i); % mean(precipp(i,:));
            if i == 1
                Msum = dmi*dmi';
            else
                Msum = Msum + dmi*dmi';
        ...
        meanrainmat(maincount,sigmaecount) = mean(ismr);
    end
end

%% Plot figure 6

figure(6), subplot(2,2,4), semilogy(monthstartlist(1:8), meanrainmat(1:8), 'kx-', 'LineWidth', 1.5), hold on,...
